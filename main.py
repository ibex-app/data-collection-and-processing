...
async def populate_collector_tasks(monitor_id:UUID, env:str, sample:bool, use_declensions:bool):
    ...
    monitor.status = MonitorStatus.sampling if sample else MonitorStatus.collecting
    await monitor.save()
    ...
    monitor = await Monitor.get(monitor_id)
    monitor.status = MonitorStatus.sampled if sample else MonitorStatus.live if monitor.date_to else MonitorStatus.finalized
    await monitor.save()
