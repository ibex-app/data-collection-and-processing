...
async def get_collector_tasks(monitor_id:UUID, env: str, sample: bool = False, use_declensions: bool = False) -> List[chain or group]:
    if sample:
    ...
    tasks_group: List[chain or group] = await to_tasks_group(collect_actions, monitor, env, sample, use_declensions)
    return tasks_group
...
def generate_hits_count_tasks(collect_action: CollectAction, 
    ...
    if search_terms and len(search_terms) > 0:
        for search_term in search_terms:
            hits_count_tasks_: List[CollectTask] = split_to_tasks(accounts, [search_term], collect_action, monitor.date_from, date_to, env, sample, use_declensions)
        ...
        hits_count_tasks_: List[CollectTask] = split_to_tasks(accounts, [], collect_action, monitor.date_from, date_to, env, sample, use_declensions )
        for hits_count_task in hits_count_tasks_:
            hits_count_task.get_hits_count = True
...
async def to_tasks_group(collect_actions: List[CollectAction], monitor: Monitor, env: str, sample:bool=False, use_declensions: bool = False) -> List[CollectTask]:
    """
